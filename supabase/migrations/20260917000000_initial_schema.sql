...
CREATE OR REPLACE FUNCTION public.update_account(
  p_avatar_url text DEFAULT '__UNCHANGED__',
  p_banner_url text DEFAULT '__UNCHANGED__',
  p_bio text DEFAULT '__UNCHANGED__',
  p_display_name text DEFAULT '__UNCHANGED__',
  p_is_private boolean DEFAULT NULL,
  p_username text DEFAULT '__UNCHANGED__',
  p_banner_position text DEFAULT '__UNCHANGED__',
  p_background_url text DEFAULT '__UNCHANGED__'
)
...
BEGIN
  ...
  SET
    avatar_url = CASE WHEN p_avatar_url = '__UNCHANGED__' THEN accounts.avatar_url ELSE p_avatar_url END,
    banner_url = CASE WHEN p_banner_url = '__UNCHANGED__' THEN accounts.banner_url ELSE p_banner_url END,
    banner_position = CASE WHEN p_banner_position = '__UNCHANGED__' THEN accounts.banner_position ELSE p_banner_position END,
    background_url = CASE WHEN p_background_url = '__UNCHANGED__' THEN accounts.background_url ELSE p_background_url END,
    bio = CASE WHEN p_bio = '__UNCHANGED__' THEN accounts.bio ELSE p_bio END,
    display_name = CASE WHEN p_display_name = '__UNCHANGED__' THEN accounts.display_name ELSE COALESCE(p_display_name, accounts.display_name) END,
    is_private = COALESCE(p_is_private, accounts.is_private),
    username = CASE WHEN p_username = '__UNCHANGED__' THEN accounts.username ELSE COALESCE(p_username, accounts.username) END,
    updated_at = now()
  WHERE id = v_user_id
  RETURNING *;
END;
