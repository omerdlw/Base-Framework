...
import { isImageIconSource, isObject, toArray } from "@/core/utils";
import { toFiniteNumber } from "@/core/utils";
...
export function resolveNextOpenState(
  ...
  if (isObject(configOrState)) {
    const config = configOrState.config || configOrState.menuConfig;
    if (config) {
      return {
        config,
...
export function resolveContextMenuPageMeta(
  ...
  const title = (navItem.contextMenuTitle ??
    navItem.title ??
    null) as ReactNode;
  const description = (navItem.contextMenuDescription ??
    navItem.description ??
    null) as ReactNode;
  const eyebrow = (navItem.contextMenuEyebrow ??
    navItem.eyebrow ??
    null) as ReactNode;
  ...
  return {
    ...
    icon,
    path:
      typeof navItem.path === "string" && navItem.path
        ? navItem.path
        : pathname,
    title,
    titleText:
