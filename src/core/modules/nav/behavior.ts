...
export function useNavigationBehavior({
  ...
  useEffect(() => {
    ...
    let activityFrameId: number | null = null;
    ...
    (event: KeyboardEvent) => {
      ...
      if (key === "ArrowDown") {
        ...
        setFocusedIndex((current) =>
          current < navigationItems.length - 1 ? current + 1 : 0,
        );
        return;
      }
      if (key === "ArrowUp") {
        event.preventDefault();
        setFocusedIndex((current) =>
          current > 0 ? current - 1 : navigationItems.length - 1,
        );
      }
    },
    [
...
export function useNavigationCompact(options: any): boolean {
  return useNavigationCompactController(options).compact;
}

export function useNavigationRouteReset(
