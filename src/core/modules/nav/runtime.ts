...
const EMPTY_NAVIGATION_DIAGNOSTICS: readonly any[] = Object.freeze([]);

function isNavigationDiagnosticsEnabled(): boolean {
  return IS_DEV;
}
...
export function createNavigationDiagnosticStore({
  ...
  return {
    ...
    record(type: string, details: Record<string, any> = {}) {
...
export function recordNavigationDiagnostic(type: string, details?: any) {
  if (!isNavigationDiagnosticsEnabled()) return;
  navigationDiagnosticStore.record(type, details);
}

let navigationInspectorSnapshot: any = null;

export function createNavigationInspectorSnapshot(
...
export function updateNavigationInspectorSnapshot(state: any) {
  if (!isNavigationDiagnosticsEnabled()) return;
...
export function createNavigationOperation(
  ...
  return {
    ...
    startedAt: Number.isFinite(Number(startedAt))
      ? Number(startedAt)
      : Date.now(),
    status: NAVIGATION_OPERATION_STATUS.PENDING as string,
    onCancel: typeof onCancel === "function" ? onCancel : null,
  };
}

function settleNavigationOperation(
  operation: any,
  status: string,
  action: any,
) {
...
export function navigationOperationReducer(state: any, action: any) {
...
export function resolveActiveNavigationOperation(state: any) {
  let active: any = null;
...
export function createNavigationSelectorStore(initialState: any = {}) {
  const listeners = new Set<() => void>();
