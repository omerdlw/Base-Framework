...
export function applyScopedCssVariables(
  ...
  for (const [cssVar, val] of Object.entries(varMap)) {
    if (cssVar && val) {
      if (!previousEntries.has(cssVar)) {
        previousEntries.set(cssVar, {
          priority: element.style.getPropertyPriority(cssVar),
          value: element.style.getPropertyValue(cssVar),
        });
      }
  ...
  return function cleanup() {
    for (const [cssVar, prev] of previousEntries.entries()) {
      if (prev.value) {
        element.style.setProperty(cssVar, prev.value, prev.priority);
      } else {
