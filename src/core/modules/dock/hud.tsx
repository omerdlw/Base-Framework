...
export const DockHud = memo(function DockHud() {
  const hud = useDockSelector((state) => state.hud);
  const { clearHud } = useDockActions();
  return <DockHudView clearHud={clearHud} hud={hud} pathname={usePathname()} />;
});

export function DockHeightSpacer({ className = "" }: { className?: string }) {
  const { dockHeight } = useDockHeight();
  return (
    <div
      aria-hidden="true"
      className={className}
      style={{ flexShrink: 0, height: dockHeight }}
    />
  );
}
