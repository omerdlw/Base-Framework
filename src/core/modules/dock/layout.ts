...
import {
  ...
  DOCK_CARD_LAYOUT,
  DOCK_HEIGHT_BUFFER,
  DOCK_SPACER_BOTTOM_LOCK_DISTANCE,
  DOCK_SURFACE_PHASE,
  VIEWPORT_MARGIN,
} from "./constants";
import {
...
export function getDockItemCardProps({
  ...
  return {
    className: cn(
      "group @container absolute h-auto w-full ring-1 ring-inset ring-white/10 bg-black/60 rounded-[30px] will-change-[transform,opacity] transition-colors duration-500",
      isShelf ? "px-1.5 py-0" : "p-2.5",
...
export function getContainerHeight({
  ...
  const minCardHeight = compact
    ? DOCK_CARD_LAYOUT.compactHeight
    : isHud
...
export function useDockHeightController({
  ...
  const applyHeight = useCallback(() => {
    ...
    const isLockedToBaseHeight =
      ...
      currentPhase === DOCK_SURFACE_PHASE.SWAPPING_HEADER ||
    ...
    const isBottomLockedForSpacer =
      ...
      canUseBottomLock() &&
      distanceToBottom <= DOCK_SPACER_BOTTOM_LOCK_DISTANCE;

    const spacerBaseHeight = isBottomLockedForSpacer
      ? DOCK_CARD_LAYOUT.compactHeight
      : height;
    const totalSpacerHeight = spacerBaseHeight + DOCK_HEIGHT_BUFFER;
  ...
  useIsomorphicLayoutEffect(() => {
    if (compact) {
      ...
      const compactHeight = DOCK_CARD_LAYOUT.compactHeight;
      const compactSpacerHeight = compactHeight + DOCK_HEIGHT_BUFFER;
      ...
      setDockHeight(compactSpacerHeight);
      return;
    }
    applyHeight();
  }, [compact, isHud, applyHeight, setDockHeight]);
