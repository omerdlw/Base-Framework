...
export function resolveModalOpenInput(
  modalInput: any,
  positionInput: any = MODAL_POSITIONS.CENTER,
  config: Record<string, any> = {},
) {
  const isDefinition =
    typeof modalInput === "object" &&
    modalInput !== null &&
    Boolean(modalInput.component || modalInput.type || modalInput.isModalDefinition);
  const def = isDefinition ? modalInput : null;
  const resolvedComponent = def
    ? def.component || null
    : typeof modalInput === "function"
      ? modalInput
      : null;
