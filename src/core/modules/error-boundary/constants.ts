...
export const ERROR_BOUNDARY_STYLES = Object.freeze({
  CONTAINER:
    "bg-white/5 ring-white/10 flex min-h-[300px] w-full flex-col items-center justify-center rounded-xl p-6 text-center ring-1 ring-inset",
  ICON_BOX:
    "bg-white/10 text-white mb-4 flex size-12 items-center justify-center rounded-full text-xl font-bold",
  TITLE: "text-foreground mb-1 text-lg font-semibold",
  DESC: "text-muted-foreground mb-4 max-w-md text-sm",
  BUTTON:
    "bg-white/10 hover:bg-white hover:text-black px-4 py-2 text-xs font-medium text-white transition-all duration-300 ease-in-out",
} as const);
