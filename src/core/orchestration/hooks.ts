...
export const useNavRegistryActions = createRegistryActionContext(
  REGISTRY_TYPES.NAV,
  DEFAULT_SOURCE,
);
...
export function useNavValue<T = any>(
  key: string,
  selector?: (value: any) => T,
  isEqual?: (a: any, b: any) => boolean,
): T {
...
export function useContextMenuRegistry() {
  ...
  const contextMenuRegister = useCallback(
    (key: string, config: any, options: any = {}) =>
      register(
        REGISTRY_TYPES.CONTEXT_MENU,
        key,
        config,
  ...
  const contextMenuUnregister = useCallback(
    (key: string, sourceOrOptions: any = DYNAMIC_SOURCE) =>
  ...
  const contextMenuBatch = useCallback(
    (executor: any) =>
      runScopedBatch(batch, executor, (queue: any) => ({
        register: (key: string, config: any, options: any = {}) =>
          queue.register(
            REGISTRY_TYPES.CONTEXT_MENU,
            key,
            config,
        ...
        unregister: (key: string, sourceOrOptions: any = DYNAMIC_SOURCE) =>
...
export function useStableDiff<T>(
  value: T,
