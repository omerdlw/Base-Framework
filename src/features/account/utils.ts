...
import { isImageIconSource, trimToNull } from "@/core/utils";
...
export function normalizeAccountPatch(
  ...
  const rawBackgroundUrl = resolveInputValue(
    input.backgroundUrl,
    input.background_url,
    null,
  );
  const rawBannerPosition = resolveInputValue(
...
export const AUTH_LAST_ACCOUNT_STORAGE_KEY = "dock_last_known_account_v2";
