...
1. **GPU Hızlandırması ve `toGpuTransform`:**
   Layout reflow (top/left/height gibi reflow maliyetli özellikler) yerine tüm uzaysal hareketler `translate3d(0, y, 0) scale(s)` ile GPU katmanında kompoze edilir. Titremeleri önlemek için `NAV_COMPOSITOR_STYLE` (`backfaceVisibility: hidden`, `WebkitFontSmoothing: antialiased`, `transform: translateZ(0)`) ve kart bazında `perspective: 1000` kullanılır. Chromium/Windows rendering titremesini kökten önlemek için `contain: paint` kullanımı kaldırılmıştır. Ayrıca `toGpuTransform` içinde `Math.round(y)` ile tamsayı piksel hizalaması (pixel snapping) yapılarak metinlerin hareket halindeyken bulanıklaşması (subpixel shimmering) önlenmiştir.
2. **Bileşik Blur Yükünün Kaldırılması:**
   Kartların `backdrop-blur-*` zemininde zaten GPU blur shader'ı çalışırken, başlık (`navHeaderSwapVariants`) ve yüzey kontrollerinde (`navSurfaceControls*Variants`) ek olarak yer alan `filter: blur(...)` özellikleri kaldırılarak sadece `opacity` ve `transform` üzerinden GPU-only kompozisyon sağlanmıştır (Windows ANGLE katmanında doku yırtılmasını ve beyaz flaşları önler).
3. **Deterministik Scheduler - Framer Motion Senkronizasyonu:**
...
4. **Kademeli Zamanlama Hiyerarşisi (`NAV_TIERS`):**
...
| **`NAV_COMPACT_STACK_ENTER_TRANSITION`**  | Kompakt moda girerken deste konteynerini hızla küçültür               | `<motion.div id="nav-card-stack">`                                          | `Nav` ([index.js](file:///Users/omerdlw/Documents/template/src/modules/nav/index.js))                                                                                                                                        | `compactDirection === "compressing"` veya `isCompactStack`                   | Deste konteynerinin 0.38s FLUID ile kompakt boyuta (38px yükseklik) pürüzsüzce daralması                                                                 |
| **`NAV_COMPACT_STACK_EXIT_TRANSITION`**   | Kompakt moddan çıkarken deste konteynerini yeniden genişletir         | `<motion.div id="nav-card-stack">`                                          | `Nav` ([index.js](file:///Users/omerdlw/Documents/template/src/modules/nav/index.js))                                                                                                                                        | `compactDirection === "restoring"`                                           | Deste konteynerinin 0.40s FLUID ile normal boyuta (68px+ yükseklik) yavaş açılması                                                                       |
...
| **`NAV_BACKDROP_TRANSITION`**             | Arkaplan karartmasının giriş ve çıkış yumuşaklığını belirler          | `<motion.div key="nav-backdrop">`                                           | `Nav` ([index.js](file:///Users/omerdlw/Documents/template/src/modules/nav/index.js#L498))                                                                                                                                   | Backdrop mount/unmount olurken                                               | Opaklık ve blur tween süresi ve eğrisi (0.84s FLUID)                                                                                                     |
...
| **`NAV_HEADER_SWAP_TRANSITION`**          | Başlık takas tween zamanlamasını kontrol eder                         | `<motion.div>`                                                              | `NavCardHeader` & `NavCardItem` ([cards.js](file:///Users/omerdlw/Documents/template/src/modules/nav/cards.js#L524, L1017))                                                                                                  | Başlık katmanı değiştiğinde                                                  | Başlık katmanının 0.52s FLUID geçişi                                                                                                                     |
...
| **`navHeaderRestoreVariants`**            | Yüzey kapandığında standart kart başlığını geri yükler                | `<motion.div key="standard-content-layer">`                                 | `NavCardItem` ([cards.js](file:///Users/omerdlw/Documents/template/src/modules/nav/cards.js#L1041-L1048))                                                                                                                    | Yüzeyden standart karta dönüşte (2. faz)                                     | Standart başlığın `blur(6px)`'den netleşerek yerine süzülmesi (-16px -> 0 / 0.52s FLUID)                                                                 |
...
| **`navSurfaceBodyVariants`**              | Açılan görev yüzeyinin (surface) gövde animasyonu                     | `<motion.div key="surface-body-motion">`                                    | `NavSurfaceShell` ([surface.js](file:///Users/omerdlw/Documents/template/src/modules/nav/surface.js#L2705-L2722))                                                                                                            | `surfacePhase === EXPANDING_BODY \|\| OPEN \|\| COLLAPSING_BODY`             | Gövdenin +20px ve `blur(10px)`'den netleşerek açılması (0.84s FLUID) ve kapanışta kademeli 1. faz olarak +24px, `blur(8px)` ile kaybolması (0.32s FLUID) |
| **`NAV_SURFACE_BODY_ENTER_TRANSITION`**   | Yüzey gövde açılış geçişi                                             | `<motion.div key="surface-body-motion">`                                    | `NavSurfaceShell` ([surface.js](file:///Users/omerdlw/Documents/template/src/modules/nav/surface.js#L2714))                                                                                                                  | Yüzey açılırken (faz 2)                                                      | 0.84s FLUID geçiş (`BODY_ENTER_MS: 840` ile senkron, kademeli ivmelenme)                                                                                 |
| **`NAV_SURFACE_BODY_EXIT_TRANSITION`**    | Yüzey gövde kapanış geçişi                                            | `<motion.div key="surface-body-motion">`                                    | `NavSurfaceShell` ([surface.js](file:///Users/omerdlw/Documents/template/src/modules/nav/surface.js#L2713))                                                                                                                  | Yüzey kapanırken (kademeli 1. faz)                                           | 0.32s FLUID geçiş (`BODY_EXIT_MS: 320` ile senkron, eşitlenmiş Apple fluid eğrisi)                                                                       |
...
| **`navSurfaceControlsContainerVariants`** | Yüzey kontrol butonları konteyner geçişi                              | `<motion.div key="nav-surface-controls-container">`                         | `NavSurfaceControls` ([surface.js](file:///Users/omerdlw/Documents/template/src/modules/nav/surface.js#L2497-L2506))                                                                                                         | `isBodyVisible && (hasClose \|\| hasBack \|\| hasHeaderAction)`              | Konteynerin `targetY` konumuna yerleşmesi (0.44s FLUID) ve çıkışı                                                                                        |
| **`navSurfaceControlsActionVariants`**    | Yüzey özel eylem butonunun girişi                                     | `<motion.div key="nav-surface-custom-action">`                              | `NavSurfaceControls` ([surface.js](file:///Users/omerdlw/Documents/template/src/modules/nav/surface.js#L2509-L2519))                                                                                                         | `hasHeaderAction === true`                                                   | Butonun `blur(4px)`'den netleşerek girişi (0.28s FLUID)                                                                                                  |
| **`navSurfaceControlsBackVariants`**      | Yüzey geri butonunun girişi                                           | `<motion.div key="nav-surface-back">`                                       | `NavSurfaceControls` ([surface.js](file:///Users/omerdlw/Documents/template/src/modules/nav/surface.js#L2522-L2543))                                                                                                         | `hasBack === true`                                                           | Butonun `blur(4px)`'den netleşerek girişi (0.32s FLUID)                                                                                                  |
| **`navSurfaceControlsCloseVariants`**     | Yüzey kapatma butonunun girişi                                        | `<motion.div key="nav-surface-close">`                                      | `NavSurfaceControls` ([surface.js](file:///Users/omerdlw/Documents/template/src/modules/nav/surface.js#L2546-L2567))                                                                                                         | `hasClose === true`                                                          | Butonun `blur(4px)`'den netleşerek pop-in girişi (0.32s FLUID)                                                                                           |
...
- **Tanım:**
  ...
  export const NAV_EASINGS = Object.freeze({
    FLUID: Object.freeze([0.82, 0, 0.18, 1]),
...
- **Teknik Amaç:** Fiziksel ve sinematik olarak doğal hissettiren eğriler sunar:
  - `FLUID`: Apple fluid arayüz algoritması. Başlangıcı son derece yavaş ve eylemsizdir ("very slow -> slow -> normal -> fast"), ardından hedefe kritik sönümleme ile yağ gibi oturur. Yüzey açılış/kapanış, başlık takası ve backdrop için kullanılır.
...
- **İlişki:**
  ...
  - `HEADER_RESTORE_MS` (520ms) -> `NAV_HEADER_SWAP_TRANSITION` ve kart yüksekliği kapanışının tamamlanması (toplam 840ms / 0.84s FLUID)
...
- **Teknik Amaç:** Yüzey açılırken içeriğin dikey translate (+20px -> 0) ve optik netleşme (`filter: blur(10px) -> blur(0px)`) hareketini `FLUID` eğrisiyle (0.84s); kapanırken ise 1. aşama olarak içeriğin eşitlenmiş Apple fluid eğrisiyle aşağı süzülüp (+24px -> 0 opacity) ve yumuşakça bulanıklaşmasını (`blur(8px)`) 0.32s'de tamamlar; ardından 2. aşamada standart başlık geri yüklenir ve kart toplam 0.84s'de (`FLUID`) 72px'e kapanır.
...
- **Tanım:**
  - `NAV_SURFACE_CONTROLS_CONTAINER_TRANSITION`: `{ type: "tween", duration: 0.44, ease: NAV_EASINGS.FLUID }`
  - `NAV_SURFACE_CONTROLS_ITEM_TRANSITION`: `{ type: "tween", duration: 0.32, ease: NAV_EASINGS.FLUID }`
  - `NAV_SURFACE_CONTROLS_ACTION_TRANSITION`: `{ type: "tween", duration: 0.28, ease: NAV_EASINGS.FLUID }`
    ...
    playing: (index) => {
      const profile =
        SOUNDWAVE_FREQUENCY_PROFILES[
          index % SOUNDWAVE_FREQUENCY_PROFILES.length
        ];
      return {
...
#### `getNavItemTransition({ expanded, isStackHovered, isRestoringDeck, isCompactingDeck, position, delay })`
...
- **Yeni `isCompactingDeck`:** Kompakt moda girildiğinde (position > 0 olan kartlar) `NAV_COMPACT_CONTENT_TRANSITION` (0.16s EXIT) ile anında gizlenir; bu sayede arkadaki kartlar görünür kalırken içerikleri önce solup sonra kompakt boyut animasyonu başlar.
- **Restore Stagger:** `isRestoringDeck` aktifken position > 0 kartlar için `0.18 + position * 0.04s` gecikme uygulanır; böylece kartlar sıralı ve titreyip olmadan dönebilir.
- **Kullanıldığı Yer:** [cards.js](file:///Users/omerdlw/Documents/template/src/modules/nav/cards.js).
