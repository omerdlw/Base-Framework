...
- **Form Elements:** NEVER use raw HTML `<button>`, `<input>`, `<textarea>`. Always import and use from `@/core/primitives`:
...
- **NEVER use trailing periods (`.`) in sentences displayed to the user or developer:**
  ...
  - *Example:* `This item is currently unavailable` (NOT `...unavailable.`)
