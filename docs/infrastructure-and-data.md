...
## 7. Redis & Distributed Caching (`src/infrastructure/redis/`)

Provides a serverless, HTTP/REST-based Redis client powered by `@upstash/redis`, designed specifically for Edge and Cloudflare Workers runtime environments.

### Core Modules:

- **`src/infrastructure/redis/client.ts`**: Server-only (`import "server-only";`) singleton factory exporting `getRedisClient()`, `createRedisClient()`, and `requireRedisClient()`.
- **`src/infrastructure/redis/index.ts`**: Barrel export for clean imports.

### Configuration (`src/infrastructure/env.ts`):

- `UPSTASH_REDIS_REST_URL`: Upstash REST endpoint URL.
- `UPSTASH_REDIS_REST_TOKEN`: Upstash REST bearer token.
- Helper functions: `getUpstashRedisConfig()`, `isUpstashRedisConfigured()`, `requireUpstashRedisConfig()`.

---

## 8. Security & Rate Limiting (`src/infrastructure/security/rate-limiter.ts`)

Protects API routes from brute-force and denial-of-service attacks with hybrid distributed + in-memory resilience:

### Features:

- **Distributed Sliding Window:** When Upstash Redis is configured (`UPSTASH_REDIS_REST_URL` and `UPSTASH_REDIS_REST_TOKEN`), uses `@upstash/ratelimit` across the entire cluster.
- **Graceful In-Memory Fallback:** If Upstash Redis is unconfigured (e.g. local dev without Upstash credentials) or experiences a temporary network blip, the system automatically falls back to high-performance in-memory sliding window tracking with zero developer friction.
- **Async & Sync API:**
  - `checkRateLimitAsync(key, options)` / `checkDistributedRateLimit`: Async distributed check with automatic in-memory fallback.
  - `checkRateLimit(key, options)`: Synchronous in-memory rate limiting.
